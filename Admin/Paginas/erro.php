<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <link rel="icon" type="image/png" href="../Imagens/logo.ico">
    <title>Mão Na Massa</title>
    <link rel="stylesheet" href="../Bibliotecas/bootstrap-3.3.7/css/bootstrap.min.css">
    <link href="../CSS/erro.css" rel="stylesheet">
</head>

<body>
<!-- Preloader -->
<section id="wrapper" class="error-page">
    <div class="error-box">
        <div class="error-body text-center">
            <h1 class="text-danger">404</h1>
            <h3 class="text-uppercase">Página Não Encontrada !</h3>
            <a href="../Paginas/login.php" class="btn btn-danger btn-rounded waves-effect waves-light m-b-40">Voltar</a> </div>
        <footer class="footer text-center">2017 © Mão na Massa.</footer>
    </div>
</section>
<script src="../Bibliotecas/JQuery/jquery-3.2.1.min.js"></script>
<script src="../Bibliotecas/bootstrap-3.3.7/js/bootstrap.min.js"></script>
</body>

</html>
