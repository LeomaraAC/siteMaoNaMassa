...
class PDF extends FPDF{
    ...
    function Header(){
        ...
        $this->Cell(60);
        // Titulo
        $this->Cell(90,25,"Relatório do Estoque de Materiais",0,0,'C'); //largura da célula, altura da célula, conteudo, borda, ... , centralizada
        // Pula uma linha
        $this->Ln(35);
    }
    function Footer()
    ...
    function tabela($cabecalho, $dados){
        ...
        $this->SetFont('','B');
        // Header
        $w = array(27, 77, 42, 42);//Tamanhos das colunas
        ...
        while ($linha = $dados->fetch_assoc())
        {
            /*if ($linha["qtde"] <= $linha["estoqueMin"]){
                $this->SetFillColor(253, 206, 207);
                $fill = true;
            }
            else
                $this->SetFillColor(209, 216, 225);*/

            $this->Cell($w[0],6,$linha["idMateriaP"],'LR',0,'C',$fill);
            $this->Cell($w[1],6,$linha["descricao"],'LR',0,'C',$fill);
            $this->Cell($w[2],6,$linha["estoqueMin"],'LR',0,'C',$fill);
            $this->Cell($w[3],6,$linha["qtde"],'LR',0,'C',$fill);
            $this->Ln();
            $fill = !$fill;
        }
        // Closing line
...
$pdf = new PDF();
$header = array('ID', 'Descrição', 'Qtde Miníma', 'Qtde Atual');
$data = $pdf->lerDados();
$pdf->PageNo();
